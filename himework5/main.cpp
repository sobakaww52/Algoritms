...
tuple<map<string, Coord>, vector<EdgeItem>> read_graphml(const string& file_path) {
    ...
    for (tinyxml2::XMLElement* edge = graph->FirstChildElement("edge"); edge; edge = edge->NextSiblingElement("edge")) {
        ...
        for (tinyxml2::XMLElement* data = edge->FirstChildElement("data"); data; data = data->NextSiblingElement("data")) {
            ...
            if (strcmp(key, "d13") == 0 && data->GetText()) { // street name (d13)
    ...
    cout << "Count nodes " << nodes.size() << " edges " << edges.size() << endl;
...
void visualize_path_with_network(const map<string, Coord>& nodes,
    ...
    sf::RenderWindow window(sf::VideoMode(figsize.x, figsize.y), "The shortest route");
...
int main() {
    ...
    map<string, Coord> nodes;
    vector<EdgeItem> edges;
    std::tie(nodes, edges) = read_graphml("C:\\Users\\glebasjanl\\Desktop\\miigaik_gleb\\C++\\homework5\\build\\bin\\Debug\\saraevo.graphml");
    //  print_edges(edges);
      // 2. Задаём названия улиц для начала и конца маршрута
    Coord start, end;
    string name;
    std::tie(start, end, name) = edges[edges.size() - 1];
    string start_street_query = "Begluk";      // Название улицы для старта (Emerika Bluma)
    string end_street_query = "Firduz-begova";      // Название улицы для финиша (Reisa Fehima Spahe)
    ...
    if (start_index == -1 || end_index == -1) {
        std::cout << "Couldn't find the specified street for the start or end of the route" << std::endl;
    }
    else {
        ...
        if (path.empty()) {
            ...
            std::cout << "Algorithm time: " << total_sec << " sec" << std::endl;
            std::cout << "Found the path of the length " << distance << " km" << std::endl;
            std::cout << "Street on the path: ";
            bool first = true;
